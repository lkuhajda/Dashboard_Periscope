...
*/
  ...
  DECLARE @FiscalYear INT = [Calendar_Year|datepart(yy, getdate())]
	DECLARE @FiscalMonthStart varchar(2) = 7
...
, witwRevenue AS
 ...
 WHERE (
		 (@ReportMonth >= @FiscalMonthStart AND(t1.CalendarMonth <= @ReportMonth AND t1.CalendarYear < @FiscalYear))
		OR
		 (@ReportMonth < @FiscalMonthStart AND(t1.CalendarMonth <= @ReportMonth  OR  t1.CalendarYear < @FiscalYear))
		)

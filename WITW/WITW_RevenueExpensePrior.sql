...
*/
	...
	DECLARE @CalendarMonth_Prev varchar(2) = [Calendar_Month|datepart(mm, getdate())]
	DECLARE @CalendarYear_Prev varchar(4) = [Calendar_Year|datepart(yy, getdate())]
  DECLARE @Month_Name varchar(12) =  DateName( month , DateAdd( month , cast(@CalendarMonth_Prev as int) , 0 ) - 1 )
...
,order_table as
(
select concat('FY', right(@FiscalYear_Prev,2), ' YTD REV - ', @Month_Name) as type, (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'REVENUE') as value
union
select concat('FY', right(@FiscalYear_Prev,2), ' YTD EXP - ', @Month_Name) as type, (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'EXPENSE')
union
select concat('FY', right(@FiscalYear_Prev,2), ' YTD NET INCOME/(LOSS)') as type, (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'REVENUE') - (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'EXPENSE')
)
select * from order_table
order by case when [type] LIKE '%REV%' then 1 when [type] LIKE '%EXP%' then 2 else 3 end

-- select 'FY15 YTD REV - May' as type, (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'REVENUE') as value
-- union
-- select 'FY15 YTD EXP - May', (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'EXPENSE')
-- union
-- select 'FY15 YTD NET INCOME/(LOSS)', (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'REVENUE') - (select CumulativeSum from result_table where CalendarMonth = @CalendarMonth_Prev and segment = 'EXPENSE')